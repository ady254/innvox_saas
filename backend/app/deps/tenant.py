...
from ..schemas.tenant import TenantState
...
async def get_current_user_for_tenant(request: Request, db: AsyncSession) -> User:
    ...
    if jwt_client_id is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token missing tenant context")

    if int(jwt_client_id) != tenant.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Token tenant (ID {jwt_client_id}) does not match current tenant (ID {tenant.id})"
        )

    res = await db.execute(select(User).where(User.id == user_id, User.client_id == tenant.id))
    user = res.scalar_one_or_none()
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found in this tenant context")
